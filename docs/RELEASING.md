...
- すべての version surfaces ( `package.json` / `package-lock.json` / `tauri.conf.json` / `Cargo.toml` / バンドル内 Info.plist ) を新バージョンに同期
...
```bash
NEW_VERSION="0.2.1"
sed -i '' "s/\"version\": \"0.2.0\"/\"version\": \"$NEW_VERSION\"/" package.json
sed -i '' "s/\"version\": \"0.2.0\"/\"version\": \"$NEW_VERSION\"/" package-lock.json
sed -i '' "s/\"version\": \"0.2.0\"/\"version\": \"$NEW_VERSION\"/" src-tauri/tauri.conf.json
sed -i '' "s/version = \"0.2.0\"/version = \"$NEW_VERSION\"/" src-tauri/Cargo.toml
# src/App.tsx の APP_VERSION も同じ考え方で更新
```

置換後、 `rg '0\.2\.0' package.json package-lock.json src-tauri/tauri.conf.json src-tauri/Cargo.toml` で取りこぼしがないか確認。
...
タグ push で `.github/workflows/release.yml` を使う場合も、workflow は draft Release を作る。
手動で作る場合は、以下のように必ず `--draft` を付ける。
...
`package.json` / `package-lock.json` / `tauri.conf.json` / `Cargo.toml` のいずれかを更新し忘れると、 DMG ファイル名とバンドル内 Info.plist のバージョンが古いままになる。 **ステップ 1 の一括置換 + `rg` での取りこぼし確認** を必ずやる。
...
- `.github/workflows/release.yml` — CI 経由の release workflow ( `basename` + ピリオド変換済み + draft Release )
